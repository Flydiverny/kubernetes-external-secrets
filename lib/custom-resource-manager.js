...
class CustomResourceManager {
  ...
  /**
   * Create custom resource in kubernetes cluster, if it doesn't exist.
   * @param {Object} customResourceManifest - Custom resource manifest.
   * @returns {Promise} Promise object representing custom reference.
   */
  async createResourceIfNotExists ({ customResourceManifest }) {
    const resourceName = customResourceManifest.metadata.name
    this._logger.info(`Creating custom resource ${resourceName}, if it does not already exist`)

    this._kubeClient.addCustomResourceDefinition(customResourceManifest)

    // try to create the CRD at first
    try {
      await this._createResource({ customResourceManifest })
    } catch (err) {
      // re-throw the error if request failed for a reason
      // other than 409 conflict "exists"
      if (err.statusCode !== 409) throw err
    ...
    try {
      return await this._getResource({ resourceName })
    } catch (err) {
      if (err.statusCode !== 409) throw err
    }

    throw new Error(`Unable to create or get resource ${resourceName}`)
  }
}
