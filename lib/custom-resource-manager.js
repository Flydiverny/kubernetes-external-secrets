...
class CustomResourceManager {
  ...
  /**
   * Checking access to custom resource in kubernetes cluster, and throw an error if not.
   * @param {Object} customResourceManifest - Custom resource manifest.
   * @returns {Promise} Promise object representing custom reference.
   */
  async checkAccessToResource ({ customResourceManifest }) {
    const resourceName = customResourceManifest.metadata.name
    this._logger.info(`Checking access to custom resource ${resourceName}`)

    try {
      return await this._getResource({ resourceName })
    } catch (err) {
      this._logger.info(`Could not get custom resource ${resourceName}.`)
      if (err.statusCode !== 409) throw err
    }

    throw new Error(`Unable to get resource ${resourceName}`)
  }
}
