...
class Poller {
   ...
   * @param {Object} externalSecret - ExternalSecret custom resource object.
  ...
  async _upsertKubernetesSecret () {
    ...
    try {
      await kubeNamespace.secrets.post({ body: secretManifest })
    } catch (err) {
      if (err.statusCode !== 409) throw err
      await kubeNamespace.secrets(this._name).put({ body: secretManifest })
    }
   ...
   */
  async _checkExternalSecretStatus () {
    try {
      const {
        body: {
          status: {
      ...
      const lastPollNbr = Date.parse(lastSync) || 0
      const now = Date.now()

      if (lastPollNbr > now) {
        return this._setNextPoll(this._intervalMilliseconds)
      }

      const nextPollIn = Math.max(lastPollNbr - (now - this._intervalMilliseconds), 0)
  ...
  start () {
    ...
    this._checkExternalSecretStatus()
